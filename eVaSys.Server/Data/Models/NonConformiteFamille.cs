...
{
    /// <summary>
    /// Class for NonConformiteFamille
    /// </summary>
    public class NonConformiteFamille : IMarkModification
    {
        #region Constructor
        public NonConformiteFamille()
        {
        }
        private NonConformiteFamille(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            int c = DbContext.NonConformiteFamilles.Where(q => (q.LibelleFRFR == LibelleFRFR || q.LibelleFRFR == LibelleFRFR) && q.RefNonConformiteFamille != RefNonConformiteFamille).Count();
        ...
        {
            string r = "";
            int nbLinkedData = DbContext.Entry(this).Collection(b => b.NonConformiteNonConformiteFamilles).Query().Count();
            if (nbLinkedData != 0)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
