...
{
    /// <summary>
    /// Class for ProduitGroupeReporting
    /// </summary>
    public class ProduitGroupeReporting : IMarkModification
    {
        #region Constructor
        public ProduitGroupeReporting()
        {
        }
        private ProduitGroupeReporting(ILazyLoader lazyLoader)
        {
        ...
        private ProduitGroupeReportingType _produitGroupeReportingType;
        public ProduitGroupeReportingType ProduitGroupeReportingType
        {
            get => LazyLoader.Load(this, ref _produitGroupeReportingType);
            set => _produitGroupeReportingType = value;
        }
        ...
        {
            string r = "";
            int c = DbContext.ProduitGroupeReportings.Where(q => q.Libelle == Libelle && q.RefProduitGroupeReporting != RefProduitGroupeReporting).Count();
            if (c > 0 || Libelle?.Length > 50 || (!string.IsNullOrEmpty(Couleur) && Couleur?.Length != 7))
            {
                CulturedRessources cR = new(currentCulture, DbContext);
                ...
                if (!string.IsNullOrEmpty(Couleur) && Couleur?.Length != 7) { if (r == "") { r += Environment.NewLine; } r += cR.GetTextRessource(975); }
            }
            return r;
        }
        //--------------------------------------------------------------------------------------------
        ...
        {
            string r = "";
            int nbLinkedData = DbContext.Entry(this).Collection(b => b.Produits).Query().Count();
            if (nbLinkedData != 0)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
