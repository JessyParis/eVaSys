...
{
    /// <summary>
    /// Class for ProduitGroupeReportingType
    /// </summary>
    public class ProduitGroupeReportingType : IMarkModification
    {
        #region Constructor
        public ProduitGroupeReportingType()
        {
        }
        private ProduitGroupeReportingType(ILazyLoader lazyLoader)
        {
        ...
        public ICollection<ProduitGroupeReporting> ProduitGroupeReportingProduitGroupeReportingTypes { get; set; }
        [NotMapped]
        ...
        {
            string r = "";
            int c = DbContext.ProduitGroupeReportingTypes.Where(q => q.Libelle == Libelle && q.RefProduitGroupeReportingType != RefProduitGroupeReportingType).Count();
        ...
        {
            string r = "";
            int nbLinkedData = DbContext.Entry(this).Collection(b => b.ProduitGroupeReportingProduitGroupeReportingTypes).Query().Count();
            if (nbLinkedData != 0)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
