...
namespace eVaSys.Data
{
    ...
    /// Class for Service
    /// </summary>
    public class Service : IMarkModification
    {
        #region Constructor
        public Service()
        {
        }
        private Service(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            bool existsInDB = (Utils.Utils.DbScalar("select count(*) from tbrService where RefService=" + RefService, DbContext.Database.GetDbConnection()) != "0");
            int c = DbContext.Services.Where(q => q.Libelle == Libelle).Count();
        ...
        {
            string r = "";
            int nbLinkedData = DbContext.Entry(this).Collection(b => b.ContactAdresseServiceFonctions).Query().Count();
            if (nbLinkedData != 0)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
