...
namespace eVaSys.Data
{
    ...
    /// Class for Ticket
    /// </summary>
    public class Ticket : IMarkModification
    {
        #region Constructor
        public Ticket()
        {
        }
        private Ticket(ILazyLoader lazyLoader)
        {
        ...
        [NotMapped]
        public string TicketTexteHTMLFromText
        {
            get
            {
                string r = "";
                r = System.Net.WebUtility.HtmlEncode(TicketTexte ?? "").Replace(Environment.NewLine, "<br/>");
                return r;
            }
        }
        ...
        {
            string r = "";
            bool existsInDB = (Utils.Utils.DbScalar("select count(*) from tblTicket where RefTicket=" + RefTicket, DbContext.Database.GetDbConnection()) != "0");
            int c = DbContext.Tickets.Where(q => q.Libelle == Libelle).Count();
