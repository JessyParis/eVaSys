...
{
    /// <summary>
    /// Class for FormeContact
    /// </summary>
    public class FormeContact : IMarkModification
    {
        #region Constructor
        public FormeContact()
        {
        }
        private FormeContact(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            bool existsInDB = (Utils.Utils.DbScalar("select count(*) from tbrFormeContact where RefFormeContact=" + RefFormeContact, DbContext.Database.GetDbConnection()) != "0");
            int c = DbContext.FormeContacts.Where(q => q.Libelle == Libelle).Count();
        ...
        {
            string r = "";
            int nbLinkedData = DbContext.Entry(this).Collection(b => b.Actions).Query().Count();
            if (nbLinkedData != 0)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
