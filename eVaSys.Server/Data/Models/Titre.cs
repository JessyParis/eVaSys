...
namespace eVaSys.Data
{
    ...
    /// Class for titre
    /// </summary>
    public class Titre : IMarkModification
    {
        #region Constructor
        public Titre()
        {
        }
        private Titre(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            bool existsInDB = (Utils.Utils.DbScalar("select count(*) from tbrTitre where RefTitre=" + RefTitre, DbContext.Database.GetDbConnection()) != "0");
            int c = DbContext.Titres.Where(q => q.Libelle == Libelle).Count();
        ...
        {
            string r = "";
            int nbLinkedData = DbContext.Entry(this).Collection(b => b.ContactAdresses).Query().Count();
            if (nbLinkedData != 0)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
