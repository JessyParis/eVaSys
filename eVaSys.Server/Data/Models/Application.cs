...
namespace eVaSys.Data
{
    ...
    /// </summary>
    public class Application : IMarkModification
    {
        #region Constructor
        public Application()
        {
        }
        private Application(ILazyLoader lazyLoader)
        {
        ...
        public ICollection<ClientApplicationApplication> ClientApplicationApplications { get; set; }
        [NotMapped]
        ...
        {
            string r = "";
            int c = DbContext.Applications.Where(q => q.Libelle == Libelle && q.RefApplication != RefApplication).Count();
        ...
        {
            string r = "";
            int nbLinkedData = DbContext.Entry(this).Collection(b => b.ClientApplicationApplications).Query().Count();
            if (nbLinkedData != 0)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
