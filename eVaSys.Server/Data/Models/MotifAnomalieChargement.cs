...
{
    /// <summary>
    /// Class for MotifAnomalieChargement
    /// </summary>
    public class MotifAnomalieChargement : IMarkModification
    {
        #region Constructor
        public MotifAnomalieChargement()
        {
        }
        private MotifAnomalieChargement(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            bool existsInDB = (Utils.Utils.DbScalar("select count(*) from tbrMotifAnomalieChargement where RefMotifAnomalieChargement=" + RefMotifAnomalieChargement, DbContext.Database.GetDbConnection()) != "0");
            int c = DbContext.MotifAnomalieChargements.Where(q => q.Libelle == Libelle).Count();
