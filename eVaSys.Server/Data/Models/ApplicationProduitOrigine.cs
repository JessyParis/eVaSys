...
{
    /// <summary>
    /// Class for ApplicationProduitOrigine
    /// </summary>
    public class ApplicationProduitOrigine : IMarkModification
    {
        #region Constructor
        public ApplicationProduitOrigine()
        {
        }
        private ApplicationProduitOrigine(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            int c = DbContext.ApplicationProduitOrigines.Where(q => q.Libelle == Libelle && q.RefApplicationProduitOrigine != RefApplicationProduitOrigine).Count();
        ...
        {
            string r = "";
            int nbLinkedData = DbContext.Entry(this).Collection(b => b.ClientApplications).Query().Count();
            nbLinkedData += DbContext.Entry(this).Collection(b => b.Produits).Query().Count();
            if (nbLinkedData != 0)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
