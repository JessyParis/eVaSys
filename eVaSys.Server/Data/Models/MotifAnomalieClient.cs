...
{
    /// <summary>
    /// Class for MotifAnomalieClient
    /// </summary>
    public class MotifAnomalieClient : IMarkModification
    {
        #region Constructor
        public MotifAnomalieClient()
        {
        }
        private MotifAnomalieClient(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            bool existsInDB = (Utils.Utils.DbScalar("select count(*) from tbrMotifAnomalieClient where RefMotifAnomalieClient=" + RefMotifAnomalieClient, DbContext.Database.GetDbConnection()) != "0");
            int c = DbContext.MotifAnomalieClients.Where(q => q.Libelle == Libelle).Count();
