...
namespace eVaSys.Data
{
    ...
    /// Class for Parametre
    /// </summary>
    public class Aide : IMarkModification
    {
        #region Constructor
        public Aide()
        {
        ...
        {
            string r = "";
            int c = DbContext.Aides.Where(q => q.Composant == Composant && q.RefAide!= RefAide).Count();
            if (c > 0 || Composant?.Length > 50)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
                if (c > 0) { if (r == "") { r += Environment.NewLine; } r += cR.GetTextRessource(410); }
                if (Composant?.Length > 50) { if (r == "") { r += Environment.NewLine; } r += cR.GetTextRessource(394); }
            }
            return r;
        }
        //--------------------------------------------------------------------------------------------
