...
{
    /// <summary>
    /// Class for SAGECodeTransport
    /// </summary>
    public class SAGECodeTransport : IMarkModification
    {
        #region Constructor
        public SAGECodeTransport()
        {
        }
        private SAGECodeTransport(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            bool existsInDB = (Utils.Utils.DbScalar("select count(*) from tbrSAGECodeTransport where RefSAGECodeTransport=" + RefSAGECodeTransport, DbContext.Database.GetDbConnection()) != "0");
            int c = DbContext.SAGECodeTransports.Where(q => q.Code == Code).Count();
            if ((!existsInDB && c > 0) || (existsInDB && c > 1) || Code?.Length > 50 || Libelle?.Length > 50)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
                if (Code?.Length > 50) { if (r == "") { r += Environment.NewLine; } r += cR.GetTextRessource(395); }
