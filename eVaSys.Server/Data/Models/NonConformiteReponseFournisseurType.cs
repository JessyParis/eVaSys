...
{
    /// <summary>
    /// Class for NonConformiteReponseFournisseurType
    /// </summary>
    public class NonConformiteReponseFournisseurType : IMarkModification
    {
        #region Constructor
        public NonConformiteReponseFournisseurType()
        {
        }
        private NonConformiteReponseFournisseurType(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            int c = DbContext.NonConformiteReponseFournisseurTypes.Where(q => (q.LibelleFRFR == LibelleFRFR || q.LibelleFRFR == LibelleFRFR) && q.RefNonConformiteReponseFournisseurType != RefNonConformiteReponseFournisseurType).Count();
            if (c > 0 || (LibelleFRFR?.Length > 50 || LibelleENGB?.Length > 50))
            {
                CulturedRessources cR = new(currentCulture, DbContext);
                if (c > 0) { if (r == "") { r += Environment.NewLine; } r += cR.GetTextRessource(410); }
                if (LibelleFRFR?.Length > 50) { if (r == "") { r += Environment.NewLine; } r += cR.GetTextRessource(399); }
                if (LibelleENGB?.Length > 50) { if (r == "") { r += Environment.NewLine; } r += cR.GetTextRessource(792); }
            }
            return r;
        }
        //--------------------------------------------------------------------------------------------
        ...
        {
            string r = "";
            int nbLinkedData = DbContext.Entry(this).Collection(b => b.NonConformites).Query().Count();
            if (nbLinkedData != 0)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
