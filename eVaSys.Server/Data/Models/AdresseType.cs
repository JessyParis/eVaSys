...
{
    /// <summary>
    /// Class for types d'adresse
    /// </summary>
    public class AdresseType : IMarkModification
    {
        #region Constructor
        public AdresseType()
        {
        }
        private AdresseType(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            //faire comme ça : int c = DbContext.AdresseTypes.Where(q => q.Libelle == Libelle && q.RefAdresseType != RefAdresseType).Count();
            bool existsInDB = (Utils.Utils.DbScalar("select count(*) from tbrAdresseType where RefAdresseType=" + RefAdresseType, DbContext.Database.GetDbConnection()) != "0");
            int c = DbContext.AdresseTypes.Where(q => q.Libelle == Libelle).Count();
