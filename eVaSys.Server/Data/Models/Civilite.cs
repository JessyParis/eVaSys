...
namespace eVaSys.Data
{
    ...
    /// Class for civilités
    /// </summary>
    public class Civilite : IMarkModification
    {
        #region Constructor
        public Civilite()
        {
        }
        private Civilite(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            int c = DbContext.Civilites.Where(q => q.Libelle == Libelle && q.RefPays == RefPays && q.RefCivilite != RefCivilite).Count();
        ...
        {
            string r = "";
            int nbLinkedData = DbContext.Entry(this).Collection(b => b.Contacts).Query().Count();
