...
{
    /// <summary>
    /// Class for types de message
    /// </summary>
    public class MessageType : IMarkModification
    {
        #region Constructor
        public MessageType()
        {
        }
        private MessageType(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            bool existsInDB = (Utils.Utils.DbScalar("select count(*) from tbrMessageType where RefMessageType=" + RefMessageType, DbContext.Database.GetDbConnection()) != "0");
            int c = DbContext.MessageTypes.Where(q => q.Libelle == Libelle).Count();
        ...
        {
            string r = "";
            int nbLinkedData = DbContext.Entry(this).Collection(b => b.Messages).Query().Count();
            if (nbLinkedData != 0)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
