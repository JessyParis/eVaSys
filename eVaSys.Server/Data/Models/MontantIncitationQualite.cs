...
{
    /// <summary>
    /// Class for MontantIncitationQualite
    /// </summary>
    public class MontantIncitationQualite : IMarkModification
    {
        #region Constructor
        public MontantIncitationQualite()
        {
        }
        private MontantIncitationQualite(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            int c = DbContext.MontantIncitationQualites.Where(q => q.Annee == Annee && q.RefMontantIncitationQualite != RefMontantIncitationQualite).Count();
            if (c > 0)
            {
                CulturedRessources cR = new(currentCulture, DbContext);
