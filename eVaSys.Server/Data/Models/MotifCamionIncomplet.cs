...
{
    /// <summary>
    /// Class for MotifCamionIncomplet
    /// </summary>
    public class MotifCamionIncomplet : IMarkModification
    {
        #region Constructor
        public MotifCamionIncomplet()
        {
        }
        private MotifCamionIncomplet(ILazyLoader lazyLoader)
        {
        ...
        {
            string r = "";
            bool existsInDB = (Utils.Utils.DbScalar("select count(*) from tbrMotifCamionIncomplet where RefMotifCamionIncomplet=" + RefMotifCamionIncomplet, DbContext.Database.GetDbConnection()) != "0");
            int c = DbContext.MotifCamionIncomplets.Where(q => q.Libelle == Libelle).Count();
