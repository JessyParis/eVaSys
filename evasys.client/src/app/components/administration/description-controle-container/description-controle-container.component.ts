...
@Component({
    selector: "description-controle-container",
    templateUrl: "./description-controle-container.component.html",
    standalone: false
})

export class DescriptionControleContainerComponent implements OnInit, OnDestroy {
  matcher = new MyErrorStateMatcher();
  //Form
  form: UntypedFormGroup;
  descriptionControles: dataModelsInterfaces.DescriptionControle[] = [];
  ...
  ngOnInit() {
    ...
    if (!isNaN(id)) {
      if (id != 0) {
        ...
        //Get data
        this.dataModelService.getDescriptionControles(id).subscribe(result => {
          if (result) {
            this.descriptionControles = result;
          }
          else {
            this.descriptionControles = [];
          }
        }, error => showErrorToUser(this.dialog, error, this.applicationUserContext));
      }
      else {
  ...
  onProduitSelected() {
    //Get data
    this.dataModelService.getDescriptionControles(this.produitListFC.value).subscribe(result => {
      if (result) {
        this.descriptionControles = result;
      }
      else {
        this.descriptionControles = [];
        this.descriptionControles = [<dataModelsInterfaces.DescriptionControle>{RefDescriptionControle: 0}];
      }
    }, error => showErrorToUser(this.dialog, error, this.applicationUserContext));
  }
  //-----------------------------------------------------------------------------------
  ...
  onAdd() {
    //Create new element
    let newElement = <dataModelsInterfaces.DescriptionControle>{
      RefDescriptionControle: 0
      , DescriptionControleProduits: [<dataModelsInterfaces.DescriptionControleProduit>{ Produit: { RefProduit: this.produitListFC.value } }]
    }
    //Add element
    this.descriptionControles.push(newElement);
  }
  //-----------------------------------------------------------------------------------
  //Copy elements
  onCopy() {
    //Process
    var url = this.baseUrl + "evapi/descriptioncontrole/copy";
    //Update 
    this.http
      .get<dataModelsInterfaces.DescriptionControle>(url, {
